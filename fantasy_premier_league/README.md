...
- **Shots, xG and xA by gameweek** (every player, ranked each week; no
  stars, no projections — just what happened) —
  <https://zenoonan.github.io/Research/fantasy_premier_league/shots.html>
  — see [Shots, xG and xA by gameweek](#shots-xg-and-xa-by-gameweek--shotspy)
- **The defcon board** (defenders and midfielders only, the same three
  categories plus defensive contributions) —
  <https://zenoonan.github.io/Research/fantasy_premier_league/defcon.html>
  — see [The defcon board](#the-defcon-board)
- **The team board** (every club's attack, defence and the difference
  between them, ranked each gameweek) —
  <https://zenoonan.github.io/Research/fantasy_premier_league/teams.html>
  — see [Team attack and defence](#team-attack-and-defence--teamspy)
...
shots.py            fbref shots x FPL xG/xA/defcon, weekly, ranked
build_shots.py      -> shots.html and defcon.html
teams.py            club attack / defence / net per gameweek, ranked
build_teams.py      -> teams.html
...
da costa*, its *Chema* is *josé maría andrés baixauli*. All three check out
on minutes to within five.
...
`defcon.html`, built by the same command, is the same board narrowed and
widened at once: **defenders and midfielders only**, on the same three
categories **plus FPL's defensive contributions**.

**Ranked against each other.** The population is cut *before* the ranking,
not after, so a defender's three shots are ranked against other defenders
and midfielders rather than against Haaland. The ranks on this page are
therefore not the ranks on `shots.html`, and are not meant to be.

**What "defensive contributions" is.** FPL's own count of qualifying
defensive actions — for a defender clearances + blocks + interceptions +
tackles; for a midfielder that plus recoveries. Verified against the dump:
every DF row equals CBI + tackles, every MD and FW row equals that plus
recoveries, every GK is 0. The board ranks the raw count, which is what the
phrase means and is the same treatment the other three get.

**The 2 points are a separate question, and are shown separately.** FPL pays
them at a threshold that differs by position — **10 for a defender, 12 for a
midfielder** — so the raw count and the reward do not line up: a midfielder's
...
3. `python build_shots.py` — writes both boards (`--board defcon` for one).
4. `python build_teams.py` — the team board. Its goals come from
   `premier_league_handicap/data/2026_2027/results.csv`, so that file needs
   the new gameweek's results too; a gameweek missing from any of the three
   sources is left off rather than half-ranked.

## Team attack and defence — `teams.py`

`teams.html` asks of clubs what the player boards ask of players, three
measures each way, ranked across the twenty clubs every gameweek:

| Side | Measures | Rank 1 is |
|---|---|---|
| **Attack** | shots taken, penalty-adjusted xG, goals scored | the most |
| **Defence** | shots conceded, penalty-adjusted xG conceded, goals conceded | the **fewest** |
| **Net** | shot difference, xG difference, goal difference (attack − defence) | the most |

Each side gets a **composite**: the three ranks summed and re-ranked 1–20,
exactly as on the player boards, with tabs for each measure showing the
value and its rank. Every cell carries the opponent and venue underneath,
since a team week means little without knowing who it was against. Tables
...
xG conceded, goals against goals conceded, composite against composite. On
**Net** the shade is the opponent's season rank on the **Net** table itself,
i.e. how strong a side it was. Red is a tough opponent and green an easy
...
season fills in. **Defence** keeps shading by the club's own week — green
top three, red bottom three.
...
* **Shots — fbref**, summed by club *inside* each cumulative sheet and then
  differenced week to week. Summing before differencing keeps a mover's
  shots with the club he took them for, since fbref splits his season by
  club.
* **xG — FPL**, summed by the club each player **played for in that
  fixture**. FPL's `team` column is his club *now*: using it would hand
  Ndiaye's two Everton weeks, and four other player-weeks, to the clubs
  those players have since joined. The fixture says which side he was on —
  a player at home was on the home team, and the home team is the opponent
  named by anyone who played away in the same fixture.
* **Goals — the handicap app's results file**, not fbref. fbref's `Gls`
  counts goals by players, so it misses own goals: seven through GW5, and
  each accounts for exactly one of the seven team-games where fbref and the
  score disagree.

Penalties follow the player boards' rule: every attempt costs the attacking
club 0.75 xG in its week, and so comes off the opponent's xG conceded too.
Goals are left as scored.

### Conceded is the opponent's for

xG and goals are per match, so that is exact. fbref's shots are only per
gameweek, so a club's shots conceded are its opponent's shots *for the
week* — exact when the opponent played once, not computable when it played
twice. A double leaves that cell blank and is named in the audit; there
have been none so far.

### Reconciliation

The audit runs on every build and is on the page, verdict first:

```
verdict: clean (0 rows to check)
gameweeks ranked: GW1, GW2, GW3, GW4, GW5
matches: 50 in the results file, 50 found in FPL's fixtures
scores agree with FPL: 50 of 50
FPL round agrees with the results gameweek: 50 of 50
clubs per gameweek: GW1 20, GW2 20, GW3 20, GW4 20, GW5 20
club shots add up to the league's every week: yes
doubles (shots conceded left blank): 0
player-weeks whose xG goes to the club he played for, not FPL's current club: 6
own goals (in the results, missed by fbref's Gls): 7
```

Beyond those lines the board is **conserved**: every shot, xG, goal and
penalty counted *for* one club is counted *against* its opponent, to the
last hundredth, and the three differences sum to zero across the league
every week. Club xG sums exactly to FPL's per-round total, and club
penalties to the nine the player boards charge.
