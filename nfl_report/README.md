...
│   ├── report_{2019..2023,2025}.csv         # generated by season_report.py
│   ├── results_{2019..2023,2025}.csv        # raw: PFR games + turnovers
│   └── odds_{2019..2023,2025}.csv           # raw: aussportsbetting odds
...
season with files in `data/`: currently **2019, 2020, 2021, 2022, 2023 and
2025** (plus the published 2015/2016). Seasons are named by **start year** (a
season runs Sept–Feb), matching `report_2016`. Note the source files are named
by the Super-Bowl calendar year, one ahead — the file labelled "2024" is the
**2023** season — and there is currently a gap at the **2024 season** (no file),
so 2025 has no predecessor.
...
weeks. This applies to 2020–2023. Seasons with no predecessor in the data
(**2019**, and **2025** because of the 2024 gap) keep a blank week 1 (LGT 0,
power neutral). Team names are normalised to current nicknames so a franchise
tracks across renames (Washington Redskins → Football Team → **Commanders**;
...
4. ~~**Prior-season carryover** — week-1 LGT and early-week power seeded from the
   previous season.~~ ✅ Done for 2020–2023.
5. **Fill the 2024 season gap** (and the 2018 file) so 2019 and 2025 also get a
   seeded week 1.
